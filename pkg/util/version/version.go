...
func parseMajorMinorPatch(version string, tolerant bool) (semver.Version, error) {
	...
	if len(groups) < 4 {
		return semver.Version{}, fmt.Errorf("failed to parse major.minor.patch from %q", version)
	}
	major, err := strconv.ParseUint(groups[1], 10, 64)
	if err != nil {
		return semver.Version{}, fmt.Errorf("failed to parse major version from %q: %w", version, err)
	}
	minor, err := strconv.ParseUint(groups[2], 10, 64)
	if err != nil {
		return semver.Version{}, fmt.Errorf("failed to parse minor version from %q: %w", version, err)
	}
	patch, err := strconv.ParseUint(groups[3], 10, 64)
	if err != nil {
		return semver.Version{}, fmt.Errorf("failed to parse patch version from %q: %w", version, err)
	}
