...
2. 执行命令： ./wrktest.sh diff zero.dat http.dat

> Note: 需要确保系统安装了 wrk 和 gnuplot 工具
EOF

zeroroot="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
wrkdir="${zeroroot}/_output/wrk"
jobname="zero"
duration="300s"
threads=$((3 * `grep -c processor /proc/cpuinfo`))

source "${zeroroot}/scripts/lib/color.sh"
...
OPTIONS:
  -h                     Usage information
  -n                     Performance test task name, default: zero
...
function zero::wrk::convert_plot_data()
{
...
function zero::wrk::prepare()
{
  rm -f ${wrkdir}/${datfile}
}

# Plot according to gunplot data file
function zero::wrk::plot() {
  gnuplot <<  EOF
...
zero::wrk::start_performance_test() {
  zero::wrk::prepare
...
case $1 in
  "diff")
    ...
    zero::wrk::plot_diff `basename $2` `basename $3`
    exit 0
    ;;
  *)
    if [ "$#" -lt 1 ];then
    ...
    zero::wrk::setup
    zero::wrk::start_performance_test "${url}"
    ;;
esac
