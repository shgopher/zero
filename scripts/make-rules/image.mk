...
IMAGES_DIR ?= $(wildcard ${ZROOT}/cmd/*)
# Determine images names by stripping out the dir names
IMAGES ?= $(filter-out tools,$(foreach image,${IMAGES_DIR},$(notdir ${image})))

ifeq (${IMAGES},)
  $(error Could not determine IMAGES, set ZROOT or run in source dir)
endif
...
image.dockerfile.%: ## Generate specified dockerfiles.
	$(eval IMAGES := $(lastword $(subst ., ,$*)))
	@$(ZROOT)/scripts/gen-dockerfile.sh $(GENERATED_DOCKERFILE_DIR) $(IMAGES)
	@if [[ "$(V)" == 1 ]]; then                                     \
		echo "DBG: Generating Dockerfile at $(GENERATED_DOCKERFILE_DIR)/$(IMAGES)" ; \
	fi; 
...
.PHONY: image.build.%
ifneq (${MULTISTAGE},1)
image.build.%: image.dockerfile.% go.build.% ## Build specified docker image.
...
image.build.%: image.dockerfile.% ## Build specified docker image in multistage way.
	...
	@DST_DIR=$(DST_DIR) $(ZROOT)/build/docker/$(IMAGE)/build.sh 2>/dev/null || true
...
image.manifest.push.multiarch.%:
	...
	REGISTRY_PREFIX=$(REGISTRY_PREFIX) PLATFROMS="$(PLATFORMS)" IMAGE=$* VERSION=$(VERSION) DOCKER_CLI_EXPERIMENTAL=enabled \
	  $(ZROOT)/build/lib/create-manifest.sh
