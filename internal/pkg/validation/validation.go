...
// validator implement the validate.ICustomValidator interface.
...
var ProviderSet = wire.NewSet(New, wire.Bind(new(validate.ICustomValidator), new(*validator)))

// New create and initialize the custom validator.
func New(cv CustomValidator) *validator {
	return &validator{registry: GetValidateFuncs(cv)}
}

func (vd *validator) Validate(ctx context.Context, req interface{}) error {
