...
// Modifications
//   - int64 became *appsv1beta1.Miner
...
type Miners map[string]*appsv1beta1.Miner
...
type minersByVersion []*appsv1beta1.Miner
...
type minersByCreationTimestamp []*appsv1beta1.Miner
...
func FromMiners(miners ...*appsv1beta1.Miner) Miners {
...
func FromMinerList(minerList *appsv1beta1.MinerList) Miners {
...
func ToMinerList(miners Miners) appsv1beta1.MinerList {
	ml := appsv1beta1.MinerList{}
...
func (s Miners) Insert(miners ...*appsv1beta1.Miner) {
	for i := range miners {
		if miners[i] != nil {
			m := miners[i]
			s[m.Name] = m
...
func (s Miners) Difference(miners Miners) Miners {
	return s.Filter(func(m *appsv1beta1.Miner) bool {
...
func (s Miners) SortedByCreationTimestamp() []*appsv1beta1.Miner {
...
func (s Miners) UnsortedList() []*appsv1beta1.Miner {
	res := make([]*appsv1beta1.Miner, 0, len(s))
	for _, value := range s {
		res = append(res, value)
	}
	return res
...
func newFilteredMinerCollection(filter Func, miners ...*appsv1beta1.Miner) Miners {
	ss := make(Miners, len(miners))
	for i := range miners {
		m := miners[i]
		if filter(m) {
...
func (s Miners) Oldest() *appsv1beta1.Miner {
	if len(s) == 0 {
		return nil
	}
...
func (s Miners) Newest() *appsv1beta1.Miner {
	if len(s) == 0 {
		return nil
	}
...
func (s Miners) sortedByVersion() []*appsv1beta1.Miner {
