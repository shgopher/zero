...
var listExample = templates.Examples(`
		...
		zeroctl user list --offset=0 --limit=10`)
...
func NewCmdList(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases:               []string{},
		Short:                 "Display all users in zero platform (Administrator rights required)",
		TraverseChildren:      true,
		Long:                  "Display all users in zero platform (Administrator rights required).",
		Example:               listExample,
...
func (o *ListOptions) Run(f cmdutil.Factory, args []string) error {
	users, err := o.client.ListUser(f.MustWithToken(context.Background()), o.ListUserRequest)
	if err != nil {
		return err
	}
