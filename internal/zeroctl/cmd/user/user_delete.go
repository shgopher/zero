...
func NewCmdDelete(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases:               []string{},
		Short:                 "Delete a user resource from zero platform (Administrator rights required)",
		TraverseChildren:      true,
		Long:                  "Delete a user resource from zero platform, only administrator can do this operation.",
...
func (o *DeleteOptions) Run(f cmdutil.Factory) error {
	_, err := o.client.DeleteUser(f.MustWithToken(context.Background()), o.DeleteUserRequest)
	if err != nil {
		return err
	}
