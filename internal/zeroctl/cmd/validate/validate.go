...
// Package validate is used to validate the basic environment for zeroctl to run.
package validate
...
import (
	...
	"github.com/superproj/zero/internal/zeroctl"
...
var validateExample = templates.Examples(`
		# Validate the basic environment for zeroctl to run
		zeroctl validate`)
...
func NewCmdValidate(f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Aliases:               []string{},
		Short:                 "Validate the basic environment for zeroctl to run",
		TraverseChildren:      true,
		Long:                  "Validate the basic environment for zeroctl to run.",
...
func (o *ValidateOptions) Run(f cmdutil.Factory, args []string) error {
	...
	table.SetColWidth(zeroctl.TableWidth)
