...
This will generate secretID and secretKey which can be used to sign JWT token.`)
	...
	createExample = templates.Examples(`
		...
		zeroctl secret create foo

		# Create secret with a specified expire time and description
		zeroctl secret create foo --expires=1988121600 --description="secret for zero"`)
...
func (o *CreateOptions) Run(f cmdutil.Factory, args []string) error {
	_, err := o.client.CreateSecret(f.MustWithToken(context.Background()), o.CreateSecretRequest)
	if err != nil {
		return err
	}
