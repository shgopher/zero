...
//go:generate mockgen -self_package github.com/superproj/zero/internal/gateway/biz/minerset -destination mock_minerset.go -package minerset github.com/superproj/zero/internal/gateway/biz/minerset MinerSetBiz
...
// MinerSetBiz defines functions used to handle minerset request.
...
func (b *minerSetBiz) List(ctx context.Context, namespace string, req *v1.ListMinerSetRequest) (*v1.ListMinerSetResponse, error) {
	total, list, err := b.ds.MinerSets().List(ctx, namespace, meta.ListOffset(req.Offset), meta.ListLimit(req.Limit))
	if err != nil {
...
func (b *minerSetBiz) Get(ctx context.Context, namespace, name string) (*v1beta1.MinerSet, error) {
	...
	if err != nil {
		log.Errorw(err, "Failed to get minerset", "minerset", klog.KRef(namespace, name))
		return nil, err
	}
