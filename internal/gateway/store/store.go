...
//go:generate mockgen -self_package github.com/superproj/zero/internal/gateway/store -destination mock_store.go -package store github.com/superproj/zero/internal/gateway/store IStore,ChainStore,MinerStore,MinerSetStore
...
// CoreDB returns the core gorm.DB from the datastore. If there is an ongoing transaction,
// the transaction's gorm.DB is returned instead.
func (ds *datastore) CoreDB(ctx context.Context) *gorm.DB {
