...
import (
	...
	"github.com/superproj/zero/internal/controller/miner/apis/config"
...
func (r *Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, options controller.Options, providerCluster cluster.Cluster) error {
	...
	_, err := ctrl.NewControllerManagedBy(mgr).
		For(&v1beta1.Miner{}).
		Watches(
			source.NewKindWithCache(&corev1.Pod{}, providerCluster.GetCache()),
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, reterr error) {
	...
	ctx, log, err := logutil.AddOwners(ctx, r.client, m)
	if err != nil {
...
func (r *Reconciler) reconcileDelete(ctx context.Context, m *v1beta1.Miner) (ctrl.Result, error) {
	...
	if minerutil.IsGenesisMiner(m) {
		if err := r.ProviderClient.CoreV1().Services(m.Namespace).Delete(ctx, minerutil.GetProviderServiceName(m), metav1.DeleteOptions{}); err != nil &&
	...
	waitErr := wait.PollImmediate(2*time.Second, r.podDeletionRetryTimeout, func() (bool, error) {
		if deletePodErr = r.ProviderClient.CoreV1().Pods(m.Namespace).Delete(ctx, m.Name, metav1.DeleteOptions{}); deletePodErr != nil &&
	...
	if waitErr != nil {
		...
		// If the pod deletion timeout is not expired yet, requeue the Miner for reconciliation.
...
func (r *Reconciler) shouldAdopt(m *v1beta1.Miner) bool {
	...
	if metav1.GetControllerOf(m) != nil || coreutil.HasOwner(m.GetOwnerReferences(), v1beta1.SchemeGroupVersion.String(), []string{"Chain"}) {
		return false
	}
...
func (r *Reconciler) PodToMiners(o client.Object) []ctrl.Request {
	pod, ok := o.(*corev1.Pod)
	if !ok {
