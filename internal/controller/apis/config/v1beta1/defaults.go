...
import (
	...
	"github.com/superproj/zero/internal/pkg/ports"
	cmutil "github.com/superproj/zero/internal/pkg/util/controllermanager"
...
func SetDefaults_ZeroControllerManagerConfiguration(obj *ZeroControllerManagerConfiguration) {
...
func RecommendedDefaultGenericControllerManagerConfiguration(obj *GenericControllerManagerConfiguration) {
	...
	if obj.HealthzBindAddress == "" {
		...
		obj.HealthzBindAddress = cmutil.AppendPortIfNeeded(obj.HealthzBindAddress, ports.ControllerManagerHealthzPort)
	}
	if obj.MetricsBindAddress == "" {
		obj.MetricsBindAddress = fmt.Sprintf("%s:%v", defaultMetricsAddress, ports.ControllerManagerStatusPort)
	} else {
		obj.MetricsBindAddress = cmutil.AppendPortIfNeeded(obj.MetricsBindAddress, ports.ControllerManagerStatusPort)
	}
	...
	if len(obj.LeaderElection.ResourceNamespace) == 0 {
		obj.LeaderElection.ResourceNamespace = ZeroControllerManagerDefaultLockObjectNamespace
	}
	if len(obj.LeaderElection.ResourceName) == 0 {
		obj.LeaderElection.ResourceName = ZeroControllerManagerDefaultLockObjectName
	}
}
...
func RecommendedDefaultChainControllerConfiguration(obj *ChainControllerConfiguration) {
	if obj.Image == "" {
		obj.Image = "ccr.ccs.tencentyun.com/superproj/zero-toyblc-amd64:v0.0.1"
	}
}
