...
type (
	...
	deletePriorityFunc func(miner *appsv1beta1.Miner) deletePriority
)
...
func oldestDeletePriority(miner *appsv1beta1.Miner) deletePriority {
	if !miner.DeletionTimestamp.IsZero() {
		return mustDelete
	}
	if _, ok := miner.ObjectMeta.Annotations[appsv1beta1.DeleteMinerAnnotation]; ok {
		return mustDelete
	}
...
func newestDeletePriority(miner *appsv1beta1.Miner) deletePriority {
	if !miner.DeletionTimestamp.IsZero() {
		return mustDelete
	}
	if _, ok := miner.ObjectMeta.Annotations[appsv1beta1.DeleteMinerAnnotation]; ok {
...
func randomDeletePolicy(miner *appsv1beta1.Miner) deletePriority {
	if !miner.DeletionTimestamp.IsZero() {
		return mustDelete
	}
	if _, ok := miner.ObjectMeta.Annotations[appsv1beta1.DeleteMinerAnnotation]; ok {
...
func getMinersToDeletePrioritized(filteredMiners []*appsv1beta1.Miner, diff int, fun deletePriorityFunc) []*appsv1beta1.Miner {
	if diff >= len(filteredMiners) {
		return filteredMiners
	} else if diff <= 0 {
		return []*appsv1beta1.Miner{}
	}
...
func getDeletePriorityFunc(ms *appsv1beta1.MinerSet) (deletePriorityFunc, error) {
	// Map the Spec.DeletePolicy value to the appropriate delete priority function
	switch msdp := appsv1beta1.MinerSetDeletePolicy(ms.Spec.DeletePolicy); msdp {
	case appsv1beta1.RandomMinerSetDeletePolicy:
		return randomDeletePolicy, nil
	case appsv1beta1.NewestMinerSetDeletePolicy:
		return newestDeletePriority, nil
	case appsv1beta1.OldestMinerSetDeletePolicy:
...
func isMinerHealthy(miner *appsv1beta1.Miner) bool {
	...
	podHealthyCondition := conditions.Get(miner, appsv1beta1.MinerPodHealthyCondition)
